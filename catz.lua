...
menu.Parent          = gethui() -- cloneref(game:GetService("CoreGui"))
menu.bg.pre.Text = ""
...
runService.PostSimulation:Connect(function() cursorupdate() end)
inputService.InputEnded:Connect(function(key) -- hide
    if key.KeyCode == Enum.KeyCode.End then
        ...
        inputService.MouseIconEnabled = not Cursor.Visible
...
function library:notify(text,duration)
    ...
    notify.Size = 13
    notify.Font = 2
    notify.Text = text or "Missing text"
    notify.Visible = true
    notify.ZIndex = 3
    notify.Center = true
    notify.Color = Color3.fromRGB(255,255,255)
    notify.Outline = true
    notify.Transparency = 0
    ...
    task.spawn(function()
        for i = 1, (duration / 2) * 30 do
            notify.Transparency = i / ((duration / 2) * 30)
            task.wait(1 / 60)
        end

        for i = 1, (duration / 2) * 30 do
            notify.Transparency = 1 - (i / ((duration / 2) * 30))
            task.wait(1 / 60)
        end
    end)
    task.spawn(function()
        wait(duration)
        table.remove(notifications, table.find(notifications, notify))
        for i = 1, #notifications do
            notifications[i].Position = Vector2.new(center.X,(center.Y + 150) + (i * 18))
        end
        notify:Remove()
...
function library:addTab(name)
    ...
    function tab:createGroup(pos,groupname) -- newTab[pos == 0 and "left" or "right"] 
        ...
        function group:addToggle(args)
            ...
            Toggle_Gradient.Rotation = 275
			print(Toggle_Gradient.Rotation)
            ...
            function toggle:addColorpicker(args)
                ...
                ColorPicker_Gradient1.Rotation = 275
                ...
                hue.MouseEnter:Connect(function()
                    local input = hue.InputBegan:connect(function(key)
                        if key.UserInputType == Enum.UserInputType.MouseButton1 then
                            while PostSim:wait() and inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
        ...
        function group:addSlider(args,sub)
            ...
            local function updateScroll()
                ...
                while inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and menu.Enabled do runService.PostSimulation:Wait()
...
--args.multiselect and "..." or ""
        ...
        function group:addColorpicker(args)
            ...
            ColorPicker_Gradient.Rotation = 275
