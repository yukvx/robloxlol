--
if not isfolder("Clor.dev") then
	makefolder("Clor.dev")
end
if not isfolder("Clor.dev/Images") then
	makefolder("Clor.dev/Images")
end
if not isfolder("Clor.dev/Configs") then
	makefolder("Clor.dev/Configs")
end
if not isfolder("Clor.dev/Configs/Universal") then
	makefolder("Clor.dev/Configs/Universal")
end
--
...
local _saveDirectory
local _characters
local _randomIndex
--
_saveDirectory = "Clor.dev/Images/"
...
--
local function zy(length)
    local text = ""
    for i = 5, length do
        _randomIndex = math.random(1, #_characters)
        text = text .. _characters:sub(_randomIndex, _randomIndex) -- Fixed variable name
    end
    return text
end
--
	...
	do
		...
		function image:_newImage(data, isUri)
			_taskspawn(function()
				...
				local path = _stringformat("%s%s-%s.clor", _saveDirectory, self._id, self._imageId)
				if isUri then
					local newData
